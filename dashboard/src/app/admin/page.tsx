...
export default function AdminPage() {
  ...
  const [editMaxDevices, setEditMaxDevices] = useState(100)
  const [editMaxLogins, setEditMaxLogins] = useState(2)
  ...
  const handleEditLimitsClick = (hotel: Hotel) => {
    setEditingHotel(hotel);
    setEditMaxDevices(hotel.max_devices ?? 100);
    setEditMaxLogins(hotel.max_dashboard_logins ?? 2);
    setEditError(null);
  };
  ...
  const handleSaveLimits = async () => {
    ...
    setEditError(null);
    try {
      const res = await fetch(`${API}/api/admin/hotels/${editingHotel.hotel_id}/limits`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${user?.token}`
        ...
        body: JSON.stringify({
          ...
          max_dashboard_logins: editMaxLogins
        })
      });
      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.detail || "Failed to update limits");
      }
      const updated = await res.json();
      // Optimistic update
      setHotels(hotels.map(h =>
        h.hotel_id === editingHotel.hotel_id
          ? { ...h, max_devices: updated.max_devices, max_dashboard_logins: updated.max_dashboard_logins }
          : h
      ));
      setSuccess(`✅ Limits updated for ${editingHotel.hotel_name}: ${updated.max_devices} tablets, ${updated.max_dashboard_logins} logins`);
    ...
    <main className="min-h-screen bg-slate-50 p-4 sm:p-6 font-sans text-slate-900">
                      ...
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 sm:gap-6 w-full sm:w-auto justify-between sm:justify-end">
                        <div className="flex gap-6 bg-slate-50 px-4 py-2 rounded-lg border border-slate-100 justify-center">
                          ...
                          <div className="text-center">
                            <div className="text-xs text-slate-500 uppercase font-bold tracking-wider mb-0.5">Tablets</div>
                            <div className={`font-bold text-sm ${
                              h.device_count >= (h.max_devices ?? 100)
                                ? 'text-red-600'
                                : h.device_count >= (h.max_devices ?? 100) * 0.8
                                ? 'text-amber-600'
                                : 'text-slate-900'
                            }`}>
                              {h.device_count}
                              <span className="text-slate-400 font-normal">/{h.max_devices ?? 100}</span>
                            </div>
                          </div>
                          <div className="text-center">
                        ...
                        {/* Edit Limits button */}
                        <button
                          onClick={() => handleEditLimitsClick(h)}
                          title="Edit tablet & login limits"
                          className="text-xs px-4 py-2 rounded-lg font-bold transition-all whitespace-nowrap bg-indigo-50 text-indigo-600 hover:bg-indigo-100 border border-indigo-200 flex items-center justify-center gap-1.5"
                        >
      ...
      {editingHotel && (
        <EditLimitsModal
          ...
          maxLogins={editMaxLogins}
          onMaxDevicesChange={setEditMaxDevices}
          onMaxLoginsChange={setEditMaxLogins}
          onSave={handleSaveLimits}
          onClose={() => { setEditingHotel(null); setEditError(null); }}
          loading={editLoading}
          error={editError}
        />
      )}
...
// ─────────────────────────────────────────────────────────────────────────────
// Edit Limits Modal
// Super admin sets max_devices and max_dashboard_logins per hotel
// ─────────────────────────────────────────────────────────────────────────────
function EditLimitsModal({
  ...
  onMaxLoginsChange: (v: number) => void
  onSave: () => void
  onClose: () => void
  loading: boolean
  error: string | null
}) {
  const tabletUsagePct = Math.round((hotel.device_count / maxDevices) * 100);
  const barColor =
    tabletUsagePct >= 100 ? "bg-red-500" :
    tabletUsagePct >= 80  ? "bg-amber-400" :
    "bg-emerald-500";
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-start justify-between mb-5">
          <div>
            <h2 className="text-xl font-bold text-slate-900">Edit Limits</h2>
            <p className="text-slate-500 text-sm mt-0.5">{hotel.hotel_name}</p>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Current tablet usage bar */}
        <div className="bg-slate-50 rounded-xl p-4 mb-5 border border-slate-200">
          <div className="flex justify-between items-center mb-2">
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Current Tablet Usage</span>
            <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
              tabletUsagePct >= 100 ? 'bg-red-100 text-red-700' :
              tabletUsagePct >= 80  ? 'bg-amber-100 text-amber-700' :
              'bg-emerald-100 text-emerald-700'
            }`}>{hotel.device_count}/{maxDevices} ({tabletUsagePct}%)</span>
          </div>
          <div className="w-full bg-slate-200 rounded-full h-2">
            <div
              className={`h-2 rounded-full transition-all ${barColor}`}
              style={{ width: `${Math.min(tabletUsagePct, 100)}%` }}
            />
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              max={500}
              value={maxDevices}
              onChange={e => onMaxDevicesChange(Math.max(1, parseInt(e.target.value) || 1))}
              className="w-full px-4 py-2.5 border border-slate-300 rounded-xl text-slate-900 font-semibold text-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500 transition-colors"
            />
            <span className="text-slate-400 text-sm whitespace-nowrap">tablets</span>
          </div>
          <p className="text-[11px] text-slate-400 mt-1.5">
            Max tablets that can register under this hotel. Setting below {hotel.device_count} will block new registrations.
          </p>
        </div>
        ...
        <div className="mb-5">
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              max={50}
              value={maxLogins}
              onChange={e => onMaxLoginsChange(Math.max(1, parseInt(e.target.value) || 1))}
              className="w-full px-4 py-2.5 border border-slate-300 rounded-xl text-slate-900 font-semibold text-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500 transition-colors"
            />
            <span className="text-slate-400 text-sm whitespace-nowrap">sessions</span>
          </div>
          <p className="text-[11px] text-slate-400 mt-1.5">
            Max concurrent dashboard logins allowed for this hotel.
          </p>
        </div>
        ...
        <div className="flex gap-2 flex-wrap mb-5">
          {[
            { label: "Basic", devices: 5, logins: 1 },
            { label: "Standard", devices: 20, logins: 2 },
            { label: "Premium", devices: 50, logins: 5 },
            { label: "Enterprise", devices: 200, logins: 10 },
          ].map(p => (
            <button
              key={p.label}
              onClick={() => { onMaxDevicesChange(p.devices); onMaxLoginsChange(p.logins); }}
              className="text-xs px-3 py-1.5 rounded-full border border-indigo-200 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 font-semibold transition-colors"
            >
              {p.label} ({p.devices}T / {p.logins}L)
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            disabled={loading}
            className="flex-1 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-sm font-semibold transition-colors disabled:opacity-50"
          >
            Cancel
          </button>
          <button
            onClick={onSave}
            disabled={loading}
            className="flex-1 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-xl text-sm font-semibold shadow-sm shadow-indigo-500/30 transition-all disabled:opacity-70"
          >
            {loading ? (
              <span className="flex items-center justify-center gap-2">
